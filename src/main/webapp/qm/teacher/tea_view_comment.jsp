...
<script>  
	$(function() { 
		...
		function loadDropdown(tea_no){   //提取学期   
			...
			$.ajax({
				url:"do?invoke=teacherViewComment@getTermInf",  
				type:'POST',  
				dataType:'json',  
		...
		function loadtable(term_no,tea_no){			//点击学期刷新表单学期  
			$.ajax({
				url:"do?invoke=teacherViewComment@getTableInf",  
				type:'POST',  
				dataType:'json',
				data:{
					term_no:term_no, 
				...
				success:function(rep){ 		 	  	  
						if(rep.result==0){
							...
							for(var j=0;j<tableInf.length;j++)
								{    
								var class_stu_check=tableInf[j].class_stu_check;
								if(class_stu_check==""){
									class_stu_check=0;
								}
								var table='<tr class="commentInf" name="'+tableInf[j].task_no+'">'+    
							     	 '<td>'+tableInf[j].course_name+'</td>'+ 
							      	'<td>'+tableInf[j].class_name+'</td>'+ 
							     	 '<td>'+tableInf[j].class_stu_amount+'</td>'+     
							     	 '<td>'+class_stu_check+'</td>' +   
							     	 '<td >'+ 
							...
							$('.btnfirst.circular.ui').click(function(){ //点击查看详情        
								...
								var task_no=$(this).parents(".commentInf").attr("name");
		...
		function loadDetail(task_no){  
			$.ajax({
					...
					url:"do?invoke=teacherViewComment@getTableDetail",  
					type:'POST',  
					dataType:'json', 
					data:{
						task_no:task_no, 
