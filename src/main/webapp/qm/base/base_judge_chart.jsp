...
<%@ include file="/commonjsp/header.jsp"%>
	<div id="container" >
		...
		<div class="ui green large message" id="messageinf">
			...
			<div >  
				<div class="ui vertical basic buttons dep small" id="deplist">         
				<button class="ui button dep active" name="0"><i class="checkmark green icon"></i>所有部门</button>      
...
<script>  
	$(function() {  
		...
		function loadyear(){
			$.ajax({ 
				url:"do?invoke=baseJudgeChart@getyearInf", 
				type:'POST',  
				dataType:'json',   
				success:function(rep){ 
					 if(rep.result==0){ 
						...
						$('.item.yearinf').unbind('click').click(function(){ 	//学年点击事件  
							...
							$('.ui.button.dep').removeClass('active');
							$('.ui.button.dep').find("i").remove();  
							$('.ui.button.dep').eq(0).addClass('active');
							var icon='<i class="checkmark green icon"></i>';    
							$('.ui.button.dep').eq(0).prepend($(icon));   
							 year_no=$(this).attr("name");
							 $('#tableinf').hide(); 
							 $("#divider").hide(); 
							 dep_No="0";
							loadchart(year_no,"0" ); 			//初始化图表  
		...
		function loadchart(year_no,dep_no ){ 
			$.ajax({ 
				url:"do?invoke=baseJudgeChart@getgrand",   
				type:'POST',  
				dataType:'json', 
				data:{
					year_no:year_no, 
						 ...
						 var dataStyle = {
							...
							myChart.setOption(option); 
							 myChart.on(echarts.config.EVENT.CLICK,function(item){  //点击图表  
									 $('#tableinf').show();    
									 $("#divider").show(); 
								 var t=setTimeout("window.scrollTo(0,600)",300);       
				    	    	   $('.teacherinf').remove();    
				    	    	   loaditem(item.seriesName ,chartinf);   
				    	      });  
							 
					}   
					$('.ui.button.dep').unbind('click').click(function(){//绑定部门点击事件
							...
							if($(this).hasClass("active")){ 	 				  
								...
								var icon='<i class="checkmark green icon"></i>';  
								$(this).prepend($(icon));  
								var Dep_no=$(this).attr('name'); 
								dep_No=Dep_no;
								loadchart(year_no,Dep_no );
							}
						}) 
				}
			});
		}
		 function loaditem(set , chartinf){
					...
					if(chartinf[i].grand=='良好'){
						var average=Math.round(parseFloat(chartinf[i].average)*100)/100;
						 ...
						 '<td class="teacher_no" name="'+chartinf[i].teacher_no+'">'+chartinf[i].teacher_no+'</td>'+
							...
							'<td class="teacher_title" name="'+chartinf[i].teacher_title+'">'+chartinf[i].teacher_title+'</td>'+
							'<td class="teacher_average" name="'+chartinf[i].average+'">'+average+'</td>'+   
							'<td class="teacher_grand" name="'+chartinf[i].grand+'"><div class="ui button small change teal">'+chartinf[i].grand+'</div>'+
		 ...
		 function load_teacher_detail(year_no , teacher_no){   //提取学期     
				...
				$.ajax({
					url:"do?invoke=baseJudgeChart@getTeacherInf", 
					type:'POST',  
					dataType:'json',   
					data:{ 
						year_no:year_no,   
		 ...
		 function loaddep(){
				$.ajax({ 
					url:"do?invoke=baseJudgeChart@getdepInf",  
					type:'POST',  
					dataType:'json',   
					success:function(rep){  
						console.log(rep);
						...
						for(var i=0;i<depinf.length;i++){
							var dep='<button class="ui button dep" name="'+depinf[i].dep_no+'">'+depinf[i].dep_name+'</button>';
							$('.ui.vertical.basic.buttons.dep').append($(dep));  
						}
					} 
				});
			}
		 ...
		 function changegrade(tea_no , year_no ,grade){
			 $.ajax({
					url:"do?invoke=baseJudgeChart@changegrade", 
					type:'POST',  
					dataType:'json',    
					data:{ 
						year_no:year_no,    
