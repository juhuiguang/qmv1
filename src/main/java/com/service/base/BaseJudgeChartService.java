...
public class BaseJudgeChartService {
	public ExecResult getdepInf(){ 
		String sql="SELECT dep_name , dep_no FROM base_department ORDER BY dep_sort";
		JSONResponse js=new JSONResponse();
		ExecResult result=js.getSelectResult(sql, null,"base_department");       
		return result; 
	} 
	public ExecResult getyearInf(){ 
		String sql="SELECT DISTINCT LEFT(term_startdate,4)AS year_no FROM base_term ORDER BY year_no DESC";
		JSONResponse js=new JSONResponse();
	...
	public ExecResult getgrand( String dep_no ,String year_no){ 
		String sql="";
		if(dep_no.equals("0")){
			sql="SELECT tb1.*,tb3.teacher_name FROM "+
