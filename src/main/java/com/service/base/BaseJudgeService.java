...
public class BaseJudgeService {
	...
	public ExecResult getDepInf( ){
		String sql="SELECT dep_no , dep_name FROM base_department ORDER BY dep_sort";
		JSONResponse js=new JSONResponse();
	...
	public ExecResult getteacher(String dep_no,String term_no){
		...
		for(int i=0;i<ja.size();i++){
			...
			ExecResult judge=getjudgeinf(term_no ,ja.getJSONObject(i).getString("teacher_no"),ja.getJSONObject(0).getString("teacher_title"));
